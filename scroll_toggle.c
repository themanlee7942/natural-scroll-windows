...
static void show_tray_menu(void) {
    ...
    AppendMenuA(menu, MF_STRING, IDM_TOGGLE_REMOTE,
                g_hookActive ? "Stop Remote Reverse" : "Start Remote Reverse");
...
static void refresh_status(void) {
    ...
    if (g_remoteStatus)
        SetWindowTextA(g_remoteStatus, g_hookActive ? "Status: ON" : "Status: OFF");
    if (g_remoteBtn)
